...
class FinanceService {
    ...
    async setBudget(category: string, amount: number, period: string, startDate: string, endDate: string) {
        return await FinanceModel.create('budget', { category, amount, period, start_date: startDate, end_date: endDate });
    }

    async updateBudget(id: number, category: string, amount: number, period: string, startDate: string, endDate: string) {
        return await FinanceModel.update('budget', id, { category, amount, period, start_date: startDate, end_date: endDate });
    }
    ...
    async saveAISettings(url: string, model: string, enabled: boolean, promptTx: string, promptInsight: string, promptChat: string) {
        await FinanceModel.saveAISettings({
            url, model, enabled, promptTx, promptInsight, promptChat
        });
    ...
    async getAvailableModels(url?: string) {
        ...
        if (url) await aiService.setConfig(url, 'llama2'); // dummy model for check
        else {
            const settings = await this.getAISettings();
            await aiService.setConfig(settings.url, settings.model);
        }
    ...
    async parseTransactionAI(text: string) {
        ...
        return await aiService.parseTransactionFromText(text, categories, accounts, settings.promptTx || null);
    }
