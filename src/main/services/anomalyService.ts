...
export const AnomalyService = {
    ...
    detectAnomalies(
        ...
        // 1. Check for unusual amount in category
        const stats = categoryStats.get(transaction.category);
        if (stats && stats.count >= 5) {
            const zScore = stats.stdDev > 0
                ? Math.abs(transaction.amount - stats.mean) / stats.stdDev
                : 0;
        ...
        // 2. Check for new category (first transaction in this category in last 90 days)
        const recentCutoff = new Date();
        recentCutoff.setDate(recentCutoff.getDate() - 90);

        const recentInCategory = history.filter(t =>
            t.category === transaction.category &&
            new Date(t.start_date) >= recentCutoff
        );
        ...
        // 3. Check for duplicates (same amount, category, and recent date)
    ...
    detectPriceIncreases(
        ...
        // Find previous transactions with similar description (subscription pattern)
        ...
        if (similar.length > 0) {
            const lastAmount = similar.sort((a, b) =>
