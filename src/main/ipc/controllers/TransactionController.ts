  ...
  registerRoutes(): Record<string, Route> {
    return {
      ...
      'add-transaction': async (_, data) => {
        ...
        const result = await this.getFinanceModel().create('transaction', rest, sanitizeAuditContext(_auditContext));
        // Return the full transaction object so the UI can update correctly
