...
export class BillController extends BaseController {
    registerRoutes(): Record<string, Route> {
        return {
            ...
            'get-bill-readings-paginated': (_, options) => this.getFinanceModel().getBillReadingsPaginated(options),

            'add-bill-reading': (_, data) => this.getFinanceModel().create('billReading', data),
            'update-bill-reading': (_, { id, data }) => this.getFinanceModel().update('billReading', id, data),
