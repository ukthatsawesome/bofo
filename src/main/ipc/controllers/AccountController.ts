    ...
    registerRoutes(): Record<string, Route> {
        return {
            ...
            'add-account': (_, data) => {
                ...
                return this.getFinanceModel().create('account', { ...rest, initial_balance: rest.balance || 0 }, sanitizeAuditContext(_auditContext));
            },

            'update-account': (_, data) => {
                const { _auditContext, ...rest } = data;
                return this.getFinanceModel().update('account', rest.id, rest, sanitizeAuditContext(_auditContext));
            },

            'delete-account': (_, id) => this.getFinanceModel().delete('account', id, false, { source: 'USER' }),
