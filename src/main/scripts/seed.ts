...
export async function seedDatabase(): Promise<{ success: boolean; message: string }> {
    ...
    try {
        ...
        const accounts = [
            ...
            { name: 'High Yield Savings', type: 'bank', balance: 25000, currency: 'USD', status: 'active' },
            { name: 'Cash Wallet', type: 'wallet', balance: 200, currency: 'USD', status: 'active' },
            { name: 'Investment Portfolio', type: 'investment', balance: 12000, currency: 'USD', status: 'active' },
        ];
        ...
        // =========================================================================
        // 3. TRANSACTIONS (500 over 2 years)
        // =========================================================================
        console.log('Generating Transactions...');
        const descriptions = [
            'Coffee shop', 'Uber ride', 'Amazon purchase', 'Netflix subscription',
            'Monthly salary', 'Client payment', 'Grocery shopping', 'Gas station',
            'Electric bill', 'Rent payment', 'Dinner out', 'Movie tickets',
            'Gym membership', 'Phone bill', 'Internet bill', 'Freelance project',
            'Stock dividend', 'Clothing store', 'Insurance payment', 'Taxi fare'
        ];
        ...
        for (let i = 0; i < TOTAL_RECORDS; i++) {
            const isIncome = Math.random() > 0.75; // 25% income
        ...
        for (const budget of budgets) {
            try {
                const existing = (await FinanceModel.getAllBudgets()).find(b => b.category === budget.category);
                if (!existing) {
        ...
        const goals = [
            { name: 'Emergency Fund', target_amount: 10000, current_amount: 3500, status: 'active', monthly_contribution: 500 },
            { name: 'New Car', target_amount: 30000, current_amount: 8000, status: 'active', monthly_contribution: 800 },
            { name: 'Vacation Fund', target_amount: 5000, current_amount: 1200, status: 'active', monthly_contribution: 200 },
        ];
        ...
        const recurring = [
            { name: 'Rent', category: 'Rent', amount: 1500, frequency: 'monthly', due_day: 1, is_active: 1 },
            { name: 'Netflix', category: 'Entertainment', amount: 15.99, frequency: 'monthly', due_day: 15, is_active: 1 },
            { name: 'Gym Membership', category: 'Entertainment', amount: 50, frequency: 'monthly', due_day: 5, is_active: 1 },
            { name: 'Phone Plan', category: 'Utilities', amount: 80, frequency: 'monthly', due_day: 20, is_active: 1 },
            { name: 'Car Insurance', category: 'Transport', amount: 600, frequency: 'yearly', due_day: 1, is_active: 1 },
        ];

        for (const charge of recurring) {
            try {
                const existing = (await FinanceModel.getAllRecurringCharges()).find(r => r.name === charge.name);
                if (!existing) {
        ...
        // =========================================================================
        // 7. BILL TYPES & READINGS
        // =========================================================================
        console.log('Creating Bill Types and Readings...');
        const billTypes = [
            { name: 'Electricity', unit_name: 'kWh', cost_per_unit: 0.12, icon: 'zap', color: '#f59e0b' },
            { name: 'Water', unit_name: 'Gallons', cost_per_unit: 0.005, icon: 'droplet', color: '#3b82f6' },
            { name: 'Natural Gas', unit_name: 'Therms', cost_per_unit: 1.20, icon: 'flame', color: '#ef4444' },
        ];
        ...
        for (const billType of billTypes) {
            try {
                ...
                // Create 4 monthly readings for each bill type
        ...
        const message = `🎉 Seeding complete in ${duration.toFixed(2)}s!\n` +
