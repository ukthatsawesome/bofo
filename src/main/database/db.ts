...
import { addColumnIfNotExists, addColumns, createIndex, createIndexes, createTrigger, hasColumn, getColumnNames } from './migrations/utils';
...
const MIGRATIONS: Migration[] = [
    ...
    },
    {
        ...
        up: async () => {
            await addColumns({ run, get, all }, 'accounts', [
            ...
            await run("UPDATE accounts SET initial_balance = balance WHERE initial_balance = 0 OR initial_balance IS NULL");
        }
    },
    {
        id: 3,
        ...
        up: async () => {
            const added = await addColumns({ run, get, all }, 'budgets', [
    ...
    },
    {
        ...
        up: async () => {
            await addColumns({ run, get, all }, 'bill_types', [
    ...
    },
    {
        ...
        up: async () => {
            ...
            `);
            await createIndex(run, 'idx_exchange_rates_pair', 'exchange_rates', ['from_currency', 'to_currency']);
    ...
    },
    {
        ...
        up: async () => {
            await createIndexes(run, [
                ...
                // Composite indexes
                { name: 'idx_transactions_active_date', table: 'transactions', columns: ['is_active', 'start_date DESC'] },
                { name: 'idx_transactions_account_date', table: 'transactions', columns: ['account_id', 'start_date DESC'] },
                { name: 'idx_transactions_type_date', table: 'transactions', columns: ['type', 'start_date DESC'] },
                { name: 'idx_transactions_category_date', table: 'transactions', columns: ['category', 'start_date DESC'] },
                // Other indexes
                ...
                { name: 'idx_contributions_date', table: 'goal_contributions', columns: 'contributed_at DESC' }
            ]);
    ...
    },
    {
        ...
        up: async () => {
            await addColumns({ run, get, all }, 'transactions', [
                { name: 'exchange_rate', definition: 'REAL DEFAULT 1' },
                { name: 'to_amount', definition: 'REAL' }
            ], log);

            // Backfill existing transfers
            await run("UPDATE transactions SET to_amount = amount WHERE type = 'transfer' AND to_amount IS NULL");
    ...
    },
    {
        ...
        up: async () => {
            ...
            await createTrigger(run, 'trg_audit_tx_insert', `
            ...
            await createTrigger(run, 'trg_audit_tx_update', `
            ...
            await createTrigger(run, 'trg_audit_tx_delete', `
    ...
    },
    {
        ...
        up: async () => {
            ...
            if (await addColumnIfNotExists({ run, get, all }, 'transactions', 'category_id', 'INTEGER REFERENCES categories(id)', log)) {
                await run(`
            ...
            if (await addColumnIfNotExists({ run, get, all }, 'budgets', 'category_id', 'INTEGER REFERENCES categories(id)', log)) {
                await run(`UPDATE budgets SET category_id = (SELECT c.id FROM categories c WHERE c.name = budgets.category AND c.type = 'expense')`);
                await run(`UPDATE budgets SET category_id = (SELECT c.id FROM categories c WHERE c.name = budgets.category LIMIT 1) WHERE category_id IS NULL`);
            }

            // Add category_id to recurring_charges
            if (await addColumnIfNotExists({ run, get, all }, 'recurring_charges', 'category_id', 'INTEGER REFERENCES categories(id)', log)) {
                await run(`UPDATE recurring_charges SET category_id = (SELECT c.id FROM categories c WHERE c.name = recurring_charges.category AND c.type = 'expense')`);
                await run(`UPDATE recurring_charges SET category_id = (SELECT c.id FROM categories c WHERE c.name = recurring_charges.category LIMIT 1) WHERE category_id IS NULL`);
            }
    ...
    },
    {
        ...
        up: async () => {
            ...
            const baseCurrency = await get<{ value: string }>("SELECT value FROM settings WHERE key = 'currency_base'");
            const defaultBase = baseCurrency?.value || 'USD';
            await run("UPDATE transactions SET base_currency = ? WHERE base_currency IS NULL", [defaultBase]);
            await run(`UPDATE transactions SET base_amount = ROUND(amount / COALESCE(exchange_rate, 1) * 100) / 100 WHERE base_amount IS NULL`);
    ...
    },
    {
        ...
        up: async () => {
            const tables = ['transactions', 'accounts', 'categories', 'budgets', 'goals', 'recurring_charges', 'bill_types'];
            for (const table of tables) {
                await addColumnIfNotExists({ run, get, all }, table, 'deleted_at', 'DATETIME', log);
            }
            await createIndexes(run, tables.map(t => ({ name: `idx_${t}_deleted`, table: t, columns: 'deleted_at' })));
    ...
    },
    {
        ...
        up: async () => {
            ...
            for (const table of tables) {
                if (await addColumnIfNotExists({ run, get, all }, table, 'updated_at', 'DATETIME DEFAULT CURRENT_TIMESTAMP', log)) {
                    // Try to backfill from created_at if it exists
                    try {
                        await run(`UPDATE ${table} SET updated_at = created_at WHERE updated_at IS NULL`);
                    } catch { /* created_at might not exist */ }
                }

                // Create auto-update trigger
                await createTrigger(run, `trg_${table}_updated_at`, `
    ...
    },
    {
        ...
        up: async () => {
            await addColumns({ run, get, all }, 'goal_contributions', [
            ...
            await createIndexes(run, [
                ...
                { name: 'idx_contributions_transaction', table: 'goal_contributions', columns: 'transaction_id' }
            ]);
    ...
    },
    {
        ...
        up: async () => {
            await addColumnIfNotExists({ run, get, all }, 'bill_readings', 'transaction_id', 'INTEGER REFERENCES transactions(id)', log);
    ...
    },
    {
        ...
        up: async () => {
            await addColumns({ run, get, all }, 'recurring_charges', [
                { name: 'last_transaction_id', definition: 'INTEGER REFERENCES transactions(id)' },
                { name: 'last_generated_date', definition: 'TEXT' },
                { name: 'account_id', definition: 'INTEGER REFERENCES accounts(id)' }
            ], log);
            await createIndexes(run, [
                { name: 'idx_recurring_last_tx', table: 'recurring_charges', columns: 'last_transaction_id' },
