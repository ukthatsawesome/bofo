...
const { SIMPLE_ROUTES } = require('./ipc/handlers');
 ...
 */
function handleCORS(req, res, isDev, allowedOrigins) {
    ...
    const isAllowed = allowedOrigins.has(origin) || (isDev && isLocalhost);
...
async function startWebServer() {
    ...
    try {
        ...
        const port = parseInt(settings.remote_access_port) || 5174;
        const apiKey = settings.remote_access_key || 'bofo-default-key';

        // Parse allowed origins from settings (comma-separated)
        const originsStr = settings.remote_access_origins || '';
        allowedOrigins = new Set(originsStr.split(',').map(o => o.trim()).filter(Boolean));
        ...
        console.log(`[Web Server] Allowed origins: ${[...allowedOrigins].join(', ') || '(dev mode: localhost)'}`);
        ...
        server = http.createServer(async (req, res) => {
            ...
            if (req.method === 'GET') {
                ...
                const rendererPath = path.join(app.getAppPath(), app.isPackaged ? 'dist/renderer' : 'src/renderer');
            ...
            try {
                ...
                if (!handler) {
                    ...
                    res.end(JSON.stringify({ error: `Channel '${channel}' not found or not available via web` }));
                    return;
                }
                ...
                console.error(`[Web Server] Error from ${clientIP}:`, err.message);
...
module.exports = { startWebServer, stopWebServer, restartWebServer };
