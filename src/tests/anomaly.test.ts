...
describe('Anomaly Detection', () => {
    describe('detectAnomalies', () => {
        ...
        it('should return empty array for normal transactions', () => {
            ...
            // Provide history showing Food is a known category
            const history: Transaction[] = [
                createTransaction({ id: 99, amount: 20, start_date: new Date().toISOString().split('T')[0] })
            ];
        ...
        it('should detect unusual amount when > 2σ from mean', () => {
            const tx = createTransaction({ amount: 500 }); // Way above normal
            const history: Transaction[] = [];
            const stats = new Map<string, CategoryStats>([
        ...
        it('should detect high severity for > 3σ deviation', () => {
            const tx = createTransaction({ amount: 1000 }); // Extremely high
            const history: Transaction[] = [];
            const stats = new Map<string, CategoryStats>([
