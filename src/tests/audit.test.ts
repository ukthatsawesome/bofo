...
describe('Audit System Integration', () => {
  ...
  it('should log Transaction update with AI source', async () => {
    ...
    await FinanceModel.update(
      ...
      tx.id,
      { category: 'Coffee' },
      { source: 'AI', metadata: { confidence: 0.99 } }
    );

    const logs = await new Promise<any[]>((resolve, reject) => {
      db.all(
        'SELECT * FROM transaction_history WHERE transaction_id = ? ORDER BY id DESC',
        [tx.id],
        (err: any, rows: any[]) => {
          if (err) reject(err);
          else resolve(rows);
  ...
  it('should log Generic Entity (Account) to audit_logs', async () => {
    const acc = await FinanceModel.create(
      'account',
      {
        name: 'Audit Bank',
        type: 'bank',
    ...
    const logs = await new Promise<any[]>((resolve, reject) => {
      db.all(
        "SELECT * FROM audit_logs WHERE entity_type = 'account' AND entity_id = ?",
        [acc.id],
        (err: any, rows: any[]) => {
          if (err) reject(err);
          else resolve(rows);
