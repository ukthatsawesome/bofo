...
const { EntityValidators, validateAmount, sanitizeData, isSafeFieldName } = require('../utils/validators');
...
const ENTITY_SCHEMAS = {
    transaction: {
        ...
        fields: ['account_id', 'to_account_id', 'type', 'category', 'amount', 'description',
            'attachment', 'frequency', 'start_date', 'end_date', 'currency', 'tags', 'is_active'],
        defaults: { currency: 'USD', frequency: 'once', is_active: 1, tags: '' },
    ...
    category: {
        ...
        orderBy: 'type, name',
        checkInUse: async (id) => {
            const cat = await get(`SELECT name FROM categories WHERE id = ?`, [id]);
            if (!cat) return false;
            const row = await get(`SELECT COUNT(*) as count FROM transactions WHERE category = ?`, [cat.name]);
            return row.count > 0;
        }
    },
    budget: {
    ...
    billReading: {
        ...
        afterWrite: async (data) => {
            ...
            if (billType?.auto_transaction && billType.category_name) {
                ...
                if (!accountId) {
                    const defaultAcc = await get(`SELECT id FROM accounts WHERE type IN ('bank', 'wallet') AND status = 'active' LIMIT 1`);
...
const FinanceModel = {
    /**
     * Create a new entity
     * @param {string} entityType - Entity type from ENTITY_SCHEMAS
     ...
     * @param {string} entityType - Entity type
     * @param {number} id - Entity ID
     * @param {boolean} force - Skip in-use check
     * @returns {Promise<{id: number, changes: number}>}
     */
    delete: async (entityType, id, force = false) => {
        const schema = ENTITY_SCHEMAS[entityType];
        if (!schema) throw new Error(`Unknown entity type: ${entityType}`);
     ...
     * @param {number} id - Entity ID
     */
    unarchive: async (entityType, id) => {
        return await FinanceModel.update(entityType, id, { status: 'active' });
    },

    // =========================================================================
    // SPECIALIZED OPERATIONS (Cannot be genericized)
    // =========================================================================

    /**
     * Sync account balance from all transactions.
     * NOTE: Database triggers (trg_balance_after_*) now handle this automatically
     * for INSERT/UPDATE/DELETE operations. This method is kept for:
     * - Manual recalculation requests
     * - Data import/restore operations
        ...
        const agg = await get(`
            SELECT 
                COALESCE(SUM(CASE 
                    ...
                    WHEN type = 'transfer' AND to_account_id = ? THEN ROUND(amount * 100) / 100
                    ELSE 0 
                END), 0) as total_in,
                COALESCE(SUM(CASE 
    ...
    getTransactionsPaginated: async (options = {}) => {
        ...
        const countResult = await get(`SELECT COUNT(*) as total FROM transactions ${whereClause}`, params);
        const data = await all(
            `SELECT * FROM transactions ${whereClause} ORDER BY ${sortBy} ${sortOrder} LIMIT ? OFFSET ?`,
            [...params, limit, offset]
        );

        return { data, total: countResult.total, limit, offset, hasMore: offset + data.length < countResult.total };
    },
    ...
    getTransactionCount: async (options = {}) => {
        ...
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        const result = await get(`SELECT COUNT(*) as count FROM transactions ${whereClause}`, params);
        return result.count;
    },
    ...
    getBillReadingsPaginated: async (options = {}) => {
        ...
        const countResult = await get(`SELECT COUNT(*) as total FROM bill_readings r ${whereClause}`, params);
        const data = await all(`
            SELECT r.*, t.name as bill_name, t.unit_name, t.cost_per_unit as current_cost_per_unit, 
                   t.color, t.icon, t.category_name
            FROM bill_readings r
    ...
    getBillProjections: async () => {
        ...
        for (const type of billTypes) {
            const stats = await get(`SELECT AVG(units_used) as avg_units, AVG(total_cost) as avg_cost FROM bill_readings WHERE bill_type_id = ?`, [type.id]);
            const lastActual = await get(`SELECT SUM(total_cost) as total FROM bill_readings WHERE bill_type_id = ? AND strftime('%Y', date) = ? AND strftime('%m', date) = ?`, [type.id, String(lastYear), String(lastMonth).padStart(2, '0')]);
            const currentActual = await get(`SELECT SUM(total_cost) as total FROM bill_readings WHERE bill_type_id = ? AND strftime('%Y', date) = ? AND strftime('%m', date) = ?`, [type.id, String(curYear), String(curMonth).padStart(2, '0')]);
    ...
    getExchangeRate: async (fromCurrency, toCurrency) => {
        ...
        const direct = await get(`SELECT rate FROM exchange_rates WHERE from_currency = ? AND to_currency = ?`, [fromCurrency, toCurrency]);
        if (direct) return direct.rate;
        const inverse = await get(`SELECT rate FROM exchange_rates WHERE from_currency = ? AND to_currency = ?`, [toCurrency, fromCurrency]);
    ...
    getAccountsWithConvertedBalances: async (baseCurrency) => {
        const accounts = await FinanceModel.getAll('account');
        const result = [];
        for (const acc of accounts) {
            const convertedBalance = await FinanceModel.convertCurrency(acc.balance, acc.currency, baseCurrency);
            result.push({ ...acc, converted_balance: convertedBalance, base_currency: baseCurrency, conversion_available: convertedBalance !== null });
        }
        return result;
    },
    ...
    exportData: async () => {
        return {
            ...
            accounts: await FinanceModel.getAll('account'),
            categories: await FinanceModel.getAll('category'),
            settings: await FinanceModel.getAllSettings(),
            budgets: await FinanceModel.getAll('budget'),
            goals: await FinanceModel.getAll('goal'),
            goalContributions: await all(`SELECT * FROM goal_contributions`),
            recurringCharges: await FinanceModel.getAll('recurringCharge'),
    ...
    exportAllToCSV: async () => {
        ...
        sections.push(tableToCSV(await FinanceModel.getAll('account'), ['id', 'name', 'type', 'balance', 'initial_balance', 'currency', 'status']));
        sections.push('\n## TRANSACTIONS');
        ...
        sections.push('\n## CATEGORIES');
        sections.push(tableToCSV(await FinanceModel.getAll('category'), ['id', 'type', 'name', 'status', 'is_default', 'color', 'icon']));
        sections.push('\n## BUDGETS');
        sections.push(tableToCSV(await FinanceModel.getAll('budget'), ['id', 'category', 'amount', 'period', 'start_date', 'end_date', 'created_at']));
        sections.push('\n## GOALS');
        sections.push(tableToCSV(await FinanceModel.getAll('goal'), ['id', 'name', 'description', 'target_amount', 'current_amount', 'monthly_contribution', 'target_date', 'status', 'priority']));
        sections.push('\n## GOAL_CONTRIBUTIONS');
        ...
        sections.push('\n## RECURRING_CHARGES');
        sections.push(tableToCSV(await FinanceModel.getAll('recurringCharge'), ['id', 'category', 'name', 'amount', 'frequency', 'due_day', 'next_due_date', 'is_active', 'notes']));
        sections.push('\n## BILL_TYPES');
    ...
    // =========================================================================
    // REMAINING SPECIALIZED METHODS (Business logic that can't be genericized)
    // =========================================================================

    // Category-specific (has custom logic for name-based lookup)
    ...
    // Goal-specific (has cascade delete and contribution tracking)
    deleteGoal: async (id) => {
