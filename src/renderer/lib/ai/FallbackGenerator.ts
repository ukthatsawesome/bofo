...
export class FallbackInsightGenerator {
    ...
    generateDashboardInsight(summary: DashboardSummary): string {
        ...
        const safeCategory = this.escapeHTML(topCategory);
        ...
        return `Net worth: ${this.formatter.formatCurrency(balance)}. Keep tracking to build your financial picture.`;
    }
    ...
    generateTransactionInsight(
        ...
        if (rate >= 30) {
            ...
            const safeTopCat = topCat ? this.escapeHTML(topCat) : 'discretionary';
        ...
        if (summary.topCategories.length > 0) {
            ...
            if (parseInt(top.percent) > 40) {
                insights.push(`${this.escapeHTML(top.category)} dominated at ${top.percent}% of spending.`);
            }
        }
    ...
    generateBudgetInsight(budgetData: BudgetSummary): string {
        ...
        if (overBudgetCategories && overBudgetCategories.length > 0) {
            insights.push(`${this.escapeHTML(overBudgetCategories[0])} is over budget – address this first.`);
        } else if (nearBudgetCategories && nearBudgetCategories.length > 0) {
            insights.push(`Watch ${this.escapeHTML(nearBudgetCategories[0])} – approaching limit.`);
        }
    ...
    generateGoalsInsight(goalsData: GoalsSummary): string {
        ...
        if (nearCompletion && nearCompletion.length > 0) {
            insights.push(
                `Almost there! "${this.escapeHTML(nearCompletion[0].name)}" is ${nearCompletion[0].progress}% complete.`
            );
        }

        if (stalled && stalled.length > 0) {
            insights.push(`"${this.escapeHTML(stalled[0].name)}" needs attention – no contributions recently.`);
        }
