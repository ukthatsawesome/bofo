...
export class AIInsightCache {
     ...
     */
    private generateKey(context: string, dataHash: string = ''): string {
    ...
    getCached(context: string, data: any = null): AIInsight | null {
        ...
        const key = this.generateKey(context, dataHash);
    ...
    invalidate(context: string | null = null): void {
        ...
        if (context) {
            this.notifySubscribers(context, null, 'invalidated');
        } else {
            this.subscribers.forEach((_, ctx) => {
                this.notifySubscribers(ctx, null, 'invalidated');
            });
        }
    }
     ...
     */
    private notifySubscribers(context: string, insight: AIInsight | null, status: string = 'ready'): void {
        const subs = this.subscribers.get(context) || [];
    ...
    async fetchInsight(
        ...
        const dataHash = this.hashData(summaryData);
        ...
        const requestPromise = (async (): Promise<AIInsight | null> => {
            try {
                ...
                this.notifySubscribers(context, insight || null, 'ready');
                return insight || null;
            } finally {
