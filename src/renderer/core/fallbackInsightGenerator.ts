/**
 * Enhanced Fallback Insight Generator
 * Provides intelligent rule-based insights when AI is unavailable
 */
...
export class FallbackInsightGenerator {
     ...
     */
    generateDashboardInsight(summary: DashboardSummary): string {
        const { balance, monthIncome, monthExpense, savingsRate, topCategory } = summary;
        const insights: Array<{ text: string, priority: number, type: 'success' | 'warning' | 'danger' | 'info' }> = [];
        ...
        // Select best insight based on priority
        insights.sort((a, b) => a.priority - b.priority);

        if (insights.length > 0) {
            // Combine top 2 insights if they're both high priority
            if (insights.length >= 2 && insights[0].priority <= 3 && insights[1].priority <= 4) {
                return this._formatInsight(insights[0].text + ' ' + insights[1].text, insights[0].type);
            }
            return this._formatInsight(insights[0].text, insights[0].type);
        }

        return this._formatInsight(
            `Net worth: ${this.formatter.formatCurrency(balance)}. Keep tracking to build your financial picture.`,
            'info'
        );
    }
     ...
     */
    generateTransactionInsight(summary: TransactionSummary, monthName: string, isPastMonth: boolean): string {
        const insights: string[] = [];
        const rate = parseFloat(summary.savingsRate);

        // Savings performance
        if (rate >= 30) {
            insights.push(`<strong class="text-success">Excellent!</strong> ${summary.savingsRate}% savings rate with ${summary.netFlow} net flow.`);
        } else if (rate >= 15) {
        ...
        // Contextual advice for ongoing month
        if (!isPastMonth && insights.length > 0) {
            const daysRemaining = this._getDaysRemainingInMonth();
    ...
    generatePlannerInsight(data: PlannerData): string {
        ...
        if (data.willGoNegative) {
            items.push(`<p class="text-danger font-semibold">⚠️ Warning: This plan will cause your balance to go negative. Consider reducing planned expenses or spreading them over a longer period.</p>`);
        }

        // Impact analysis
        const impactPercent = parseFloat(data.impactPercent);
        if (impactPercent < -50) {
            items.push(`<p>High-impact plan: reduces balance by ${Math.abs(impactPercent).toFixed(0)}% over ${data.range} months. Ensure you have contingency funds.</p>`);
        } else if (impactPercent < -30) {
            items.push(`<p>Significant impact on finances. The ${data.plannedExpenses} in planned expenses requires careful budgeting.</p>`);
        ...
        // Expense vs Income balance advice
        const hasOnlyExpenses = data.plannedExpensesRaw > 0 && data.plannedIncomeRaw === 0;
        if (hasOnlyExpenses && !data.willGoNegative) {
            items.push(`<p>Consider offsetting expenses with additional income sources or savings reallocation.</p>`);
        }

        // Frequency-based advice
        const hasRecurring = (data.items || []).some(i => i.frequency !== 'once');
        if (hasRecurring) {
            items.push(`<p>Recurring items in your plan will compound over time. Review the ${data.range}-month projection carefully.</p>`);
        }

        // Default message
        if (items.length === 0) {
            items.push(`<p>Your plan appears balanced. Continue monitoring actual spending against this projection.</p>`);
        }
     ...
     */
    generateBudgetInsight(budgetData: BudgetSummary): string {
        const { overBudgetCategories, nearBudgetCategories, underBudgetCategories, totalSpent, totalBudget } = budgetData;
        const insights: string[] = [];
        ...
        if (overallPercent > 100) {
            insights.push(`<strong class="text-danger">Over budget</strong> by ${this.formatter.formatCurrency(totalSpent - totalBudget)}. Review your spending immediately.`);
        } else if (overallPercent > 90) {
            insights.push(`At ${overallPercent.toFixed(0)}% of budget with time remaining. Consider slowing down spending.`);
     ...
     */
    generateGoalsInsight(goalsData: GoalsSummary): string {
        const { totalProgress, nearCompletion, stalled, availableForGoals } = goalsData;
        const insights: string[] = [];
        ...
        if (availableForGoals > 0) {
            insights.push(`You have ${this.formatter.formatCurrency(availableForGoals)} available for goal contributions.`);
        }
    ...
    /**
     * Format insight with HTML styling based on type
     */
    _formatInsight(text: string, type: string): string {
        ...
        return text; // More complex styling could be added here
    }

    /**
     * Get days remaining in current month
     */
    _getDaysRemainingInMonth(): number {
        const now = new Date();
...
// Factory function
export function createFallbackGenerator(formatter: Formatter): FallbackInsightGenerator {
    return new FallbackInsightGenerator(formatter);
}
