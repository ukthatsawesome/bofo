...
export class FallbackInsightGenerator {
  ...
  generateTransactionInsight(
    ...
    if (rate >= 30) {
      ...
      insights.push(
        `${summary.savingsRate}% savings rate. Consider reducing ${safeTopCat} spending.`
      );
    ...
    if (summary.topCategories.length > 0) {
      ...
      if (parseInt(top.percent) > 40) {
        insights.push(
          `${this._escapeHTML(top.category)} dominated at ${top.percent}% of spending.`
        );
      }
    }
    ...
    if (summary.yoyChange !== null) {
      ...
      if (change > 20) {
        insights.push(
          `Expenses up ${summary.yoyChange}% vs last year – review for lifestyle creep.`
        );
      } else if (change < -10) {
  ...
  generatePlannerInsight(data: PlannerData): string {
    ...
    if (impactPercent < -50) {
      ...
      items.push(
        `<p>Moderate financial impact. Monitor your actual spending against this projection.</p>`
      );
  ...
  generateBudgetInsight(budgetData: BudgetSummary): string {
    ...
    if (overallPercent > 100) {
      insights.push(
        `<strong class="text-danger">Over budget</strong> by ${this.formatter.formatCurrency(totalSpent - totalBudget)}. Review your spending immediately.`
      );
    ...
    if (overBudgetCategories && overBudgetCategories.length > 0) {
      insights.push(
        `${this._escapeHTML(overBudgetCategories[0])} is over budget – address this first.`
      );
  ...
  generateGoalsInsight(goalsData: GoalsSummary): string {
    ...
    if (nearCompletion && nearCompletion.length > 0) {
      insights.push(
        `Almost there! "${this._escapeHTML(nearCompletion[0].name)}" is ${nearCompletion[0].progress}% complete.`
      );
    }

    if (stalled && stalled.length > 0) {
      insights.push(
        `"${this._escapeHTML(stalled[0].name)}" needs attention – no contributions recently.`
      );
    }

    if (availableForGoals > 0) {
      insights.push(
        `You have ${this.formatter.formatCurrency(availableForGoals)} available for goal contributions.`
      );
    }

    return insights.length > 0
      ? insights.join(' ')
      : 'Keep contributing to reach your financial goals.';
  }
