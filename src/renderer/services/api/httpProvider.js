...
export class HttpApiProvider {
    constructor() {
        ...
        return new Proxy(this, {
            get: (target, prop) => {
                if (prop in target) return target[prop]; // Return explicit methods if defined
                ...
                return async (data) => {
                    ...
                    const channel = target._toKebabCase(prop);
    ...
    promptConfig() {
        const host = prompt("Bofo Remote Access Setup\n\nPlease enter the Host URL (e.g. http://192.168.1.10:5174):", this.host || "");
