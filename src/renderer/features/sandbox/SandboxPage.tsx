...
import {
    Zap, Car, Plane, HeartPulse, TrendingUp, LineChart,
    ListChecks, Trash2, Layout, Cpu, Info, ChevronDown
} from 'lucide-preact';
...
export const SandboxPage = () => {
    ...
    const currentBalance = financeStore.activeAccounts.value
        .reduce((sum, a) => sum + (['bank', 'wallet'].includes(a.type) ? a.balance : 0), 0);
            ...
            <div className="space-y-6">
                ...
                <UiCard className="border-dashed" variant="flat">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <TemplateButton icon={Car} title="Buy Car" value="$20,000" onClick={() => applyTemplate('car')} />
                        <TemplateButton icon={Plane} title="Vacation" value="$3,000" onClick={() => applyTemplate('vacation')} />
                        <TemplateButton icon={HeartPulse} title="Emergency" value="$5,000" onClick={() => applyTemplate('emergency')} />
                        <TemplateButton icon={TrendingUp} title="Salary Raise" value="+$500/mo" onClick={() => applyTemplate('raise')} />
                    </div>
                </UiCard>
                        ...
                        <UiCard className="p-0 overflow-hidden">
                            <div className="p-4 border-b border-border flex items-center justify-between bg-surface-base/10">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-xs font-bold text-text-muted uppercase tracking-wider">{plannedItems.length} Items</span>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="p-4 bg-surface-card rounded-xl border border-border">
                            ...
                            <p className="text-xs text-text-muted leading-relaxed">
                                Planned items are temporary and only exist in this sandbox. They help you visualize how a potential large purchase or life change will impact your net worth over time.
                            </p>
                        </div>
                    </div>
                </div>
...
const TemplateButton = ({ icon: Icon, title, value, onClick }: any) => (
    ...
    >
        ...
        <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-1">{value}</span>
    </button>
);
