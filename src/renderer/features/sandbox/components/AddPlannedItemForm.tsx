  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Input
        ...
        value={item.amount}
        onInput={(e) =>
          setItem({ ...item, amount: parseFloat((e.target as HTMLInputElement).value) })
        }
        placeholder="0.00"
        step="0.01"
        min="0"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            onChange={(e) =>
              setItem({ ...item, account_id: parseInt((e.target as HTMLSelectElement).value) })
            }
            className="w-full bg-surface-base border border-border rounded-lg px-3 py-2 text-sm text-text-primary focus:ring-2 focus:ring-brand-primary outline-none"
          >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            onChange={(e) =>
              setItem({ ...item, frequency: (e.target as HTMLSelectElement).value as any })
            }
            className="w-full bg-surface-base border border-border rounded-lg px-3 py-2 text-sm text-text-primary focus:ring-2 focus:ring-brand-primary outline-none"
          >
