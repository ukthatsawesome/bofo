...
export const GoalsPage = () => {
  ...
  const handleContributeSubmit = async (e: Event) => {
    ...
    if (contributeGoalId) {
      await contribute(
        ...
        contributeForm.source,
        contributeForm.notes
      );
  ...
  const HeaderActions = (
    <div className="flex gap-3">
      ...
      <UiButton icon={<Plus size={18} />} onClick={handleNewGoal}>
        New Goal
      </UiButton>
    </div>
  );
      ...
      <div className="space-y-6">
        ...
        {summary && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <UiStatCard label="Active Goals" value={summary.activeGoals} icon={Target} />
            <UiStatCard
              label="Total Saved"
                ...
                <div>
                  ...
                  <p
                    className={clsx(
                      ...
                      available.available >= 0 ? 'text-success' : 'text-danger'
                    )}
                  >
              ...
              <div className="text-right text-sm text-text-muted space-y-1 bg-surface-base/50 p-3 rounded-lg border border-border/50">
                <div className="flex justify-between gap-8">
                  ...
                  <span className="font-mono text-text-primary">
                    {formatCurrency(available.avgMonthlyIncome)}
                  </span>
                </div>
                <div className="flex justify-between gap-8">
                  <span>- Fixed:</span>{' '}
                  <span className="font-mono text-text-primary">
                    {formatCurrency(available.recurringCharges)}
                  </span>
                </div>
                <div className="flex justify-between gap-8">
                  <span>- Goals:</span>{' '}
                  <span className="font-mono text-text-primary">
                    {formatCurrency(available.goalContributions)}
                  </span>
                </div>
              </div>
            </div>
        ...
        {goals.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
            ...
            <UiButton onClick={handleNewGoal} icon={<Plus size={16} />}>
              Create Goal
            </UiButton>
          </div>
        ) : (
        ...
        >
          <form onSubmit={handleGoalSubmit} className="space-y-4">
            <Input
              ...
              value={goalForm.name}
              onInput={(e) =>
                setGoalForm({ ...goalForm, name: (e.target as HTMLInputElement).value })
              }
              required
            ...
            <Input
              ...
              onInput={(e) =>
                setGoalForm({ ...goalForm, description: (e.target as HTMLInputElement).value })
              }
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input
                ...
                onInput={(e) =>
                  setGoalForm({
                    ...goalForm,
                    target_amount: parseFloat((e.target as HTMLInputElement).value),
                  })
                }
                required
                min="1"
              ...
              <Input
                ...
                onInput={(e) =>
                  setGoalForm({
                    ...goalForm,
                    monthly_contribution: parseFloat((e.target as HTMLInputElement).value),
                  })
                }
                min="0"
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input
                ...
                onInput={(e) =>
                  setGoalForm({ ...goalForm, target_date: (e.target as HTMLInputElement).value })
                }
              />
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setGoalForm({
                      ...goalForm,
                      priority: parseInt((e.target as HTMLSelectElement).value),
                    })
                  }
                >
