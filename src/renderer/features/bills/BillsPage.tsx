...
export const BillsPage = () => {
    const { readings, billTypes, isLoading, stats, deleteReading, addReading, updateReading } = useBills();
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1">
                        <UiCard title="Recent Readings" noPadding className="h-full">
                            <DataTable
                                data={readings.slice(0, 5)} // Show top 5
                                isLoading={isLoading}
                                keyField="id"
                                columns={[
                                    {
                ...
                <UiCard title="Reading History">
                    <DataTable
                        ...
                        keyField="id"
                        columns={[
                            { header: 'Date', accessor: (row: any) => row.reading_date ? new Date(row.reading_date).toLocaleDateString() : 'N/A' },
                            ...
                            { header: 'Amount', accessor: (row: any) => formatCurrency(row.amount) },
                            {
                                ...
                                accessor: (row: any) => (
                                    <div className="flex gap-1 justify-end">
                                        <IconButton icon={Edit3} variant="primary" tooltip="Edit" onClick={() => handleEdit(row)} />
                                        <IconButton icon={Trash2} variant="danger" tooltip="Delete" onClick={() => deleteReading(row.id)} />
                                    </div>
                                ),
                ...
                {/* Bill Reading Modal */}
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingReading ? 'Edit Bill Reading' : 'New Bill Reading'}>
                    <form onSubmit={handleSave} className="space-y-4">
                        <UiSelect
                            label="Bill Type"
                            options={billTypes.map((t: any) => ({ label: t.name, value: t.id }))}
                            value={formData.bill_type_id}
                            onChange={(e) => setFormData({ ...formData, bill_type_id: (e.target as HTMLSelectElement).value })}
                            required
                        />
                        <div className="grid grid-cols-2 gap-4">
                            <Input
                                ...
                                onInput={(e) => setFormData({ ...formData, reading_date: (e.target as HTMLInputElement).value })}
                                required
                            />
                            <Input
                                label="Total Cost"
                                ...
                                onInput={(e) => setFormData({ ...formData, total_cost: (e.target as HTMLInputElement).value })}
                                required
                            />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <Input
                                ...
                                onInput={(e) => setFormData({ ...formData, units_consumed: (e.target as HTMLInputElement).value })}
                            />
                            <Input
                                label="Notes"
                        ...
                        <div className="flex gap-3 pt-4 border-t border-border mt-6">
                            <UiButton type="button" variant="ghost" className="flex-1" onClick={() => setIsModalOpen(false)}>
                                Cancel
                            </UiButton>
