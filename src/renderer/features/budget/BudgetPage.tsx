      ...
      <div className="space-y-6">
        ...
        {budgets.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
            ...
            <p className="text-text-muted mb-4">Set a budget to track your spending limits.</p>
            <UiButton onClick={handleNew} icon={<Plus size={16} />}>
              Create Budget
            </UiButton>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {budgets.map((b: any) => (
              <BudgetCard key={b.id} budget={b} onEdit={handleEdit} onDelete={deleteBudget} />
            ))}
          </div>
        )}
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            <UiSelect
              ...
              onChange={(e) =>
                setFormData({ ...formData, category: (e.target as HTMLSelectElement).value })
              }
              required
            />
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input
                ...
                onInput={(e) =>
                  setFormData({ ...formData, amount: Number((e.target as HTMLInputElement).value) })
                }
                required
              />
              <UiSelect
