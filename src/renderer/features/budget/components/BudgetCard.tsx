...
export const BudgetCard = ({ budget, onEdit, onDelete }: BudgetCardProps) => {
    ...
    return (
        <UiCard className={clsx(
            ...
            budget.status === 'over' ? "border-l-4 border-l-danger" :
                budget.status === 'warning' ? "border-l-4 border-l-warning" : "border-l-4 border-l-success"
        )}>
            <div className="flex justify-between items-start mb-4">
                <div className="flex gap-4">
                    <div className={clsx(
                        "w-12 h-12 rounded-xl flex items-center justify-center transition-colors",
                        budget.status === 'over' ? "bg-danger/10 text-danger" :
                            budget.status === 'warning' ? "bg-warning/10 text-warning" : "bg-success/10 text-success"
                    )}>
                        <PieChart size={24} />
                    </div>
                    <div>
