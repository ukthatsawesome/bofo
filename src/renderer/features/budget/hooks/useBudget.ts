...
export function useBudget() {
    ...
    const filteredBudgets = useMemo(() => {
        ...
        return budgets.filter(b => {
            if (filter === 'active') {
                ...
                const month = parseInt(mStr); // 1-indexed month

                const monthStart = `${year}-${String(month).padStart(2, '0')}-01`;
                const lastDay = new Date(year, month, 0).getDate(); // month is 1-indexed, so new Date(year, month, 0) gives last day of month-1
    ...
    // Fetch spending data when view changes
    useEffect(() => {
    ...
    // Compute final list with progress
