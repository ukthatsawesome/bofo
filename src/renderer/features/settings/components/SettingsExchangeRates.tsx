...
export const SettingsExchangeRates = () => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (provider === 'custom') {
        await (window as any).api.updateSetting({
          key: SETTING_KEYS.CURRENCY.CUSTOM_URL,
          value: customUrl,
        });
      }

      const result = await (window as any).api.syncExchangeRates({
        provider,
        baseCurrency,
        customUrl,
      });
  ...
  const handleTest = async () => {
    setLoading(true);
    try {
      const result = await (window as any).api.testCurrencyAPI({
        provider,
        baseCurrency,
        customUrl,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <UiCard>
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <span className="text-brand-primary font-bold">{baseCurrency}</span>
          </div>
          {lastSync && (
            <div
              className={clsx(
                'flex items-center gap-2 px-3 py-1.5 rounded text-sm',
                lastSync.isStale ? 'bg-warning/10 text-warning' : 'bg-success/10 text-success'
              )}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          ...
          <div className="flex gap-2">
            ...
            <UiButton onClick={handleSync} isLoading={loading} icon={<RefreshCw size={16} />}>
              Sync Now
            </UiButton>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={async (e) => {
                ...
                setAutoSync(val);
                await (window as any).api.updateSetting({
                  key: SETTING_KEYS.CURRENCY.AUTO_SYNC,
      ...
      <Modal
        ...
        title={editingRate?.id ? 'Edit Rate' : 'Add Manual Rate'}
      >
        <form onSubmit={handleSaveRate} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onInput={(e) =>
                setEditingRate({
                  ...editingRate,
                  from_currency: (e.target as HTMLInputElement).value,
                })
              }
              required
            />
            <Input
              ...
              onInput={(e) =>
                setEditingRate({
                  ...editingRate,
                  to_currency: (e.target as HTMLInputElement).value,
                })
              }
              required
            />
          ...
          <Input
            ...
            onInput={(e) =>
              setEditingRate({ ...editingRate, rate: Number((e.target as HTMLInputElement).value) })
            }
            required
          />
