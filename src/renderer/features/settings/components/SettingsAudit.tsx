...
import { ClipboardList, Sparkles, User, Trash2, PlusCircle, Edit2, AlertCircle } from 'lucide-preact';
...
export const SettingsAudit = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                // Handle both paginated response and direct array (backward compatibility)
    ...
    const getDescription = (log: AuditLog) => {
        ...
        if (log.entity_type === 'transaction') {
            if (log.action === 'CREATE') actionText = `created transaction for ${log.changes?.category || 'unknown'}`;
        ...
        return (
            <span>
                <span className={clsx("font-bold", isAI ? "text-brand-secondary" : "text-brand-primary")}>{actor}</span>
    ...
    const getChangeDetails = (log: AuditLog) => {
        ...
        const changes = Object.keys(newVal).filter(k =>
            JSON.stringify(newVal[k]) !== JSON.stringify(old[k]) && !['updated_at', 'created_at'].includes(k)
        );
        ...
        return (
            <div className="flex flex-wrap gap-2 mt-2">
                {changes.map(key => (
                    <span key={key} className="text-xs px-2 py-0.5 bg-surface-base border border-border rounded text-text-muted">
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {filteredLogs.map(log => {
                        ...
                        return (
                            <div key={uniqueKey} className="flex gap-4 p-4 rounded-xl border border-border/50 bg-surface-card hover:border-border transition-colors">
                                <div className={clsx(
                                    "shrink-0 w-10 h-10 rounded-full flex items-center justify-center",
                                    isAI ? "bg-brand-secondary/10 text-brand-secondary" : "bg-brand-primary/10 text-brand-primary"
                                )}>
                                    <Icon size={18} />
                                </div>
