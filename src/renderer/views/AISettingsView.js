/**
 * AISettingsView - Standalone AI Configuration View
 * 
 * This view handles all AI/Ollama configuration settings.
 * Accessed via the AI status indicator in the sidebar.
 */
...
export class AISettingsView extends BaseView {
    ...
    async loadAISettings() {
        ...
        $('#set-ai-url').value = aiSettings.url || 'http://localhost:11434';
        $('#set-ai-enabled').checked = aiSettings.enabled !== false;
        $('#set-ai-prompt-tx').value = aiSettings.promptTx || '';
        $('#set-ai-prompt-insight').value = aiSettings.promptInsight || '';
        $('#set-ai-prompt-chat').value = aiSettings.promptChat || '';
    ...
    bindEvents(aiSettings) {
        ...
        $('#reset-prompt-tx')?.addEventListener('click', async () => {
            ...
            if (defaults.promptTx) $('#set-ai-prompt-tx').value = defaults.promptTx;
        });

        $('#reset-prompt-insight')?.addEventListener('click', async () => {
            const defaults = await window.api.getAIDefaults();
            if (defaults.promptInsight) $('#set-ai-prompt-insight').value = defaults.promptInsight;
        });

        $('#reset-prompt-chat')?.addEventListener('click', async () => {
            const defaults = await window.api.getAIDefaults();
            if (defaults.promptChat) $('#set-ai-prompt-chat').value = defaults.promptChat;
        });
    }

    async refreshModels(aiSettings) {
        ...
        try {
            ...
            const models = await window.api.getOllamaModels(url);
    ...
    async updateHealthStatus() {
        ...
        try {
            const health = await window.api.getAIHealth();
    ...
    async saveSettings() {
        const settings = {
            url: $('#set-ai-url')?.value.trim() || 'http://localhost:11434',
            model: $('#set-ai-model')?.value || '',
            enabled: $('#set-ai-enabled')?.checked ?? true,
            promptTx: $('#set-ai-prompt-tx')?.value || '',
            promptInsight: $('#set-ai-prompt-insight')?.value || '',
            promptChat: $('#set-ai-prompt-chat')?.value || ''
        };
...
window.AISettingsView = AISettingsView;
