...
export class GoalsView extends BaseView {
    ...
    openGoalModal(goalId: number | null = null): void {
        ...
        if (goalId) {
            ...
            if (targetBtn) targetBtn.classList.add('border-brand-primary', 'text-brand-primary', 'bg-brand-primary/10');
        }
    ...
    async submitContribution(): Promise<void> {
        ...
        try {
            ...
            this.app.notifications.toast('Contribution Added',
                `${this.app.formatter.formatCurrency(amount)} added to "${goal?.name || 'Goal'}"`, 'success');
