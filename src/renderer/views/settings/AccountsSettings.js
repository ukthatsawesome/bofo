/**
 * Settings - Accounts Management Section
...
export const AccountsSettingsMixin = {
        ...
        notifications.modal({
            ...
            content: `
                <div class="form-group">
                    ...
                    <input type="text" id="acc-name" class="form-control" value="${account?.name || ''}" placeholder="e.g. Main Bank">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="acc-balance" class="form-control" step="0.01" value="${account?.initial_balance || 0}">
                </div>
                <div class="form-group">
                    <label>Currency</label>
                    <select id="acc-currency" class="form-control currency-select">
    ...
    async handleDeleteAccount(id) {
        ...
        if (inUse) {
            notifications.toast('Cannot Delete', 'This account has transactions. Archive it instead.', 'warning');
            return;
        }

        if (await notifications.confirm('Delete Account', `Delete "${account.name}"? This cannot be undone.`)) {
            try {
                await window.api.deleteAccount(id);
                await this.app.loadData();
                this.renderAccountsTable();
                notifications.toast('Deleted', 'Account removed', 'success');
            } catch (error) {
                notifications.toast('Error', error.message, 'error');
            }
        }
    ...
    getCurrencyOptions(selected = 'USD') {
        const currencies = ['USD', 'EUR', 'GBP', 'JPY', 'CNY', 'INR', 'NPR', 'AUD', 'CAD', 'CHF'];
