...
export class BillsView extends BaseView {
        ...
        return `
                <tr class="hover:bg-brand-primary/5 transition-colors border-b border-border last:border-0">
                    ...
                    <td class="px-5 py-4 text-sm font-bold text-text-primary">
                     ...
                     <td class="px-5 py-4 text-sm font-medium">${r.units_used || 0} ${UIUtils.escapeHTML((r as any).unit_name || '')}</td>
  ...
  renderInsights(projections: BillProjection[]): void {
    ...
    if (projections.length === 0 || !projections.some((p) => p.projected_cost > 0)) {
      container.innerHTML =
        '<p class="text-text-muted text-sm text-center py-4">No data to analyze</p>';
      return;
    }
  ...
  renderCharts(projections: BillProjection[]): void {
    ...
    if (trendCtx) {
      ...
      readings.forEach((r) => {
        if (!r.date) return; // Skip readings without a date
  ...
  handleNewReading(): void {
    if (this.app.state.billTypes.length === 0) {
      this.app.notifications.toast(
        ...
        'Please set up bill types in settings first.',
        'warning'
      );
      return;
    }
  ...
  handleEditReading(id: number | string): void {
    const reading =
      this.app.state.billReadings.find((r) => r.id == id) ||
      this.app.state.allBillReadings.find((r) => r.id == id);
    if (!reading) return;
  ...
  showReadingModal(reading: any = null): void {
    ...
    const updateUnitInfo = () => {
      ...
      if (opt) {
        ...
        $('#reading-cost-hint')!.innerText =
          `Current rate: ${this.app.formatter.formatCurrency(costPer)} per ${opt.dataset.unit}`;
      }
    };
    ...
    newSaveBtn.addEventListener('click', async () => {
      ...
      if (!data.date || data.units_used <= 0 || data.total_cost <= 0) {
        this.app.notifications.toast(
          'Validation Error',
          'Please enter valid units and cost',
          'error'
        );
        return;
      }
