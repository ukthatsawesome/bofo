...
export class SandboxView extends BaseView {
  ...
  populateDropdowns(): void {
    ...
    if (accountSelect) {
      ...
      accountSelect.innerHTML = activeAccounts
        .map(
          (a) =>
            `<option value="${a.id}">${a.name} (${formatter.formatCurrency(a.balance)})</option>`
        )
        .join('');
    }
  }
  ...
  setupListeners(): void {
    document.querySelectorAll('#plan-type-toggle .segment').forEach((btn) => {
      btn.addEventListener('click', () => {
        document
          .querySelectorAll('#plan-type-toggle .segment')
  ...
  addPlannedItem(): void {
    const activeSegment = document.querySelector(
      '#plan-type-toggle .segment.active'
    ) as HTMLElement;
