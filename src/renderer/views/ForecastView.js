...
export class ForecastView extends BaseView {
    ...
    async render() {
        ...
        if (customRange && timeline.length > 0) {
            ...
            timeline = timeline.filter(d => d.date >= startStr && d.date <= endStr);
        }
    ...
    renderSummary(summary) {
        ...
        container.innerHTML = `
            ...
            ${StatCard({
            label: 'Runway',
            value: runway === Infinity
                ? 'Infinite'
                : (runway || 0).toFixed(1) + ' Months',
            icon: 'clock'
        })}
        `;
    }
    ...
    renderInsights(insights) {
        ...
        const content = insights.map(i => FeedbackItem({
            type: i.type,
            icon: i.type === 'danger' ? 'x-circle' : (i.type === 'warning' ? 'alert-triangle' : 'check-circle'),
            title: i.title,
            message: i.message
        })).join('');
