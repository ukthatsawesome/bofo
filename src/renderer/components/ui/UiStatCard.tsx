...
const getValueSizeClass = (value: string): string => {
    ...
    if (len <= 7) return 'text-3xl';   // e.g. "100.00"
    if (len <= 10) return 'text-2xl';   // e.g. "11,479.74"
    if (len <= 13) return 'text-xl';    // e.g. "259,788.57"
    return 'text-lg';                   // very large numbers
};
...
export const UiStatCard: FunctionalComponent<StatCardProps> = ({
    ...
    return (
        <UiCard
            className={twMerge("group relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-default", className)}
            onClick={onClick}
            hoverEffect={!!onClick}
        >
            {/* Top row: label + icon */}
            <div className="flex items-start justify-between mb-3">
                ...
                <div className={twMerge("shrink-0 p-2.5 rounded-xl flex items-center justify-center shadow-inner transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3", colors[color])}>
            ...
            <div className="flex items-center justify-between mt-3">
                {trend && trendValue ? (
                    <div className={twMerge("flex items-center gap-1.5 text-sm font-bold bg-surface-active w-fit px-2 py-0.5 rounded-lg border border-border",
                ...
                {currency && (
                    <span className="text-xs font-semibold text-text-muted uppercase tracking-wider">{currency}</span>
                )}
            </div>
