...
export function ToggleButtonGroup<T extends string>({
  ...
  return (
    <div
      className={clsx('flex p-1 rounded-xl transition-all', containerVariants[variant], className)}
    >
      {options.map((option) => (
        <button
          key={option.value}
          ...
          className={clsx(
            ...
            sizes[size],
            value === option.value ? activeVariants[variant] : inactiveVariants[variant]
          )}
        >
          {option.label}
        </button>
