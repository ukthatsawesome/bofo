        ...
        <table className="w-full">
          <thead>
            <tr className="bg-surface-hover border-b border-border">
              {columns.map((col, idx) => {
                const sortIdentifier =
                  col.sortKey || (typeof col.accessor === 'string' ? col.accessor : undefined);
            ...
            {data.map((item) => (
              <tr
                ...
                className={clsx(
                  ...
                  onRowClick
                    ? 'cursor-pointer hover:bg-surface-hover focus:outline-none focus:ring-2 focus:ring-brand-primary/20'
                    : ''
                )}
              >
                {columns.map((col, idx) => (
