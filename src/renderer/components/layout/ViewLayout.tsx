...
interface ViewLayoutProps {
    title: string;
    subtitle?: string;
    breadcrumbs?: preact.ComponentChildren; // Optional for now
    ...
    return (
        <div className={`flex flex-col min-h-full ${className}`}>
            ...
            <header className="sticky top-0 z-10 bg-surface-base/80 backdrop-blur-md border-b border-border/50 px-8 py-4 mb-6 transition-all duration-200">
                <div className={`mx-auto flex items-center justify-between ${fullWidth ? 'max-w-full' : 'max-w-7xl'}`}>
            ...
            {/* Main Content */}
            <main className={`flex-1 px-8 pb-12 mx-auto w-full ${fullWidth ? 'max-w-full' : 'max-w-7xl'} ${fadeIn}`}>
                {isLoading ? (
