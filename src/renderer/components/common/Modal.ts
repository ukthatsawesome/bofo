...
export const Modal = ({
  ...
  return `
    <div id="${id}" 
         class="modal fixed inset-0 bg-black/85 backdrop-blur-sm z-[2000] flex items-center justify-center transition-all duration-300 hidden opacity-0 invisible pointer-events-none"
         role="dialog"
         aria-modal="true"
    ...
    >
        <div class="modal-content bg-surface-card w-[90%] ${sizeClass} p-10 rounded-lg border border-border-strong shadow-premium relative animate-[modalSlideUp_0.5s_cubic-bezier(0.16,1,0.3,1)]">
            ...
            ${title
      ? `
                <h2 id="${id}-title" class="text-xl font-bold text-text-main mb-6">${title}</h2>
            `
      : ''
    }
